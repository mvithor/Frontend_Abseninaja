...
const PendaftaranSekolahForm = ({ setSuccess, setError }) => {
  ...
  const mutation = useMutation({
    ...
    mutationFn: async (newPendaftaran) => {
      const response = await axiosInstance.post('/api/v1/pendaftaran', newPendaftaran);
      return response.data;
    },
    onSuccess: (data) => {
        setSuccess(data.msg);
        setError("");
        setTimeout(() => navigate('/dashboard/admin/manajemen-sekolah/pendaftaran-sekolah'), 3000);
    },
    onError: (error) => {
  ...
  const handleSubmit = (event) => {
    ...
    mutation.mutate(formState); // Kirim form state menggunakan mutation dari React Query
  };
