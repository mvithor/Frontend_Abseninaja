...
import { fetchPendaftaranSekolah,
         deletePendaftaranSekolah,
         searchPendaftaranSekolah,
         getVisiblePendaftaran,
         setEditingItem
       } from 'src/store/apps/pendaftaran-sekolah/PendaftaranSekolahSlice';
...
const PendaftaranListing = ({ sekolahId }) => {
    ...
    useEffect(() => {
       dispatch(fetchPendaftaranSekolah(sekolahId)) 
    }, [dispatch, sekolahId]);

    const pendaftaranSekolah = useSelector((state) => getVisiblePendaftaran(state));

    // Cek apakah pendaftaran sekolah adalah array
    console.log("Pendaftaran Sekolah:", Array.isArray(pendaftaranSekolah), pendaftaranSekolah);

    const handleEdit = (item) => {
        dispatch(setEditingItem(item));
        navigate(`/dashboard/admin/manajemen-sekolah/pendaftaran/edit/${item.id}`);
    };
                ...
                {Array.isArray(pendaftaranSekolah) && pendaftaranSekolah.map((item, index) => (
                    <TableRow key={item.id}>
                        ...
                        <TableCell align='center'>
                            ...
                            <Typography>-</Typography>  // Placeholder jika StatusPendaftaran tidak tersedia
                            )}
                        </TableCell>
                        <TableCell align="center">
