...
const fetchJamById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/pengaturan-jam/${id}`);
...
const PengaturanJamEdit = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (jam) => {
            return await axiosInstance.put(`/api/v1/pengaturan-jam/${id}`, jam);
        },
        onSuccess: (response) => {
        ...
        <PageContainer title="Edit Pengaturan Jam" description="Pengaturan Jam">
            <ParentCard title="Pengaturan Jam">
                ...
                <PengaturanJamEditForm
                    jamData={jamData} // Pastikan jamData selalu digunakan
