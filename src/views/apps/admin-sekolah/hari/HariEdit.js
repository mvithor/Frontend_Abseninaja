...
const fetchHariById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/hari/${id}`);
...
const HariEdit = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (hari) => {
            return await axiosInstance.put(`/api/v1/hari/${id}`, hari);
        },
        onSuccess: (response) => {
    ...
    const handleSubmit = (event) => {
        ...
        const updatedHariData = {
            ...hariData,
            tanggal_khusus: hariData.kategori_hari === "KBM" ? null : hariData.tanggal_khusus,
        };
    
        if (!updatedHariData.nama_hari || !updatedHariData.kategori_hari) {
