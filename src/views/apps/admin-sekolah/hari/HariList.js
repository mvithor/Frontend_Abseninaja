...
const fetchHari = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/hari');
...
const HariList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const response = await axiosInstance.delete(`/api/v1/hari/${id}`);
    ...
    return (
        <PageContainer title="Hari" description="Hari">
            <ParentCard title="Hari">
                ...
                />
                <AddButton
                    icon={<IconCalendar style={{ color: 'white', marginRight: '8px' }} />}
                    onClick={handleAdd}   
                >
                    Tambah Hari
                </AddButton>
