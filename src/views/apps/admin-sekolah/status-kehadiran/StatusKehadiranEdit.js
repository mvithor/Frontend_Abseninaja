...
const fetchStatusKehadiranById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/status-kehadiran/${id}`);
...
const StatusKehadiranEdit = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (statusKehadiran) => {
            const response = await axiosInstance.put(`/api/v1/status-kehadiran/${id}`, { nama_status: statusKehadiran.nama_status});
