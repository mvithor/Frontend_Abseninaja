...
import DeleteButton from "src/components/buttonGroup/DeleteButton";
...
const fetchWaliKelas = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/wali-kelas');
...
const WaliKelasList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const response = await axiosInstance.delete(`/api/v1/wali-kelas/${id}`);
