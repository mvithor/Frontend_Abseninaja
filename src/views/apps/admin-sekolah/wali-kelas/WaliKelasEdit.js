...
const fetchWaliKelasById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/wali-kelas/${id}`);
...
const WaliKelasEdit = () => {
    ...
    useEffect(() => {
        if (data) {
            console.log('Data yang diterima dari server:', data); // Debugging
            setWaliKelasData({
                pegawai_id: data.pegawai_id || "", // Pastikan data pegawai_id tersedia
                kelas_id: data.kelas_id || "", // Pastikan data kelas_id tersedia
            });
        }
    }, [data]);
    ...
    const mutation = useMutation({
        mutationFn: async (waliKelas) => {
            return await axiosInstance.put(`/api/v1/wali-kelas/${id}`, waliKelas);
        },
        onSuccess: (response) => {
