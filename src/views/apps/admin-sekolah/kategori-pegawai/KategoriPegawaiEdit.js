...
const fetchKategoriPegawaiById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/kategori-pegawai/${id}`);
...
const KategoriPegawaiEdit = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (kategori) => {
            const response = await axiosInstance.put(`/api/v1/kategori-pegawai/${id}`, { nama_kategori: kategori.nama_kategori });
