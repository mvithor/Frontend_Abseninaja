...
const fetchKategoriWaktuById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/kategori-waktu/${id}`);
...
const KategoriWaktuEdit = () => {
    ...
    const mutation = useMutation({
        mutationFn: async (kategoriWaktu) => {
            const response = await axiosInstance.put(`/api/v1/kategori-waktu/${id}`, { nama_kategori: kategoriWaktu.nama_kategori});
