...
const fetchKategoriWaktu = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/kategori-waktu');
...
const KategoriWaktuList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const response = await axiosInstance.delete(`/api/v1/kategori-waktu/${id}`);
        ...
        <PageContainer title="Kategori Waktu" description="Kategori Waktu">
            <ParentCard title="Kategori Waktu">
                ...
                <Box
                    ...
                    />
                    <AddButton
                        icon={<IconBuilding style={{ color: 'white', marginRight: '8px' }} />}
                        onClick={handleAdd}
                    >
                        Tambah Kategori
                    </AddButton>
