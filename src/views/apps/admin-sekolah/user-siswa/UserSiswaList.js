...
const fetchUserSiswa = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/users/siswa');
...
const UserSiswaList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async ({ id, type }) => {
            const response = await axiosInstance.delete(`/api/v1/users/${id}?type=siswa`);
    ...
    const handleEdit = (id) => {
        navigate(`/dashboard/admin-sekolah/user/siswa/edit/${id}`);
    };
