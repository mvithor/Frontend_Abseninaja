...
const fetchWaktu = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/waktu');
...
const WaktuList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const response = await axiosInstance.delete(`/api/v1/waktu/${id}`);
    ...
    const handleClone = () => {
        navigate('/dashboard/admin-sekolah/waktu/tambah-clone')
    }
    ...
    return (
        <PageContainer title="Waktu" description="Waktu">
            <ParentCard title="Waktu">
                ...
                />
                <AddButton
                    icon={<IconClock style={{ color: 'white', marginRight: '8px' }} />}
                    onClick={handleAdd}   
                >
                    Tambah Jam
                </AddButton>
