...
const fetchWaktuById = async (id) => {
    try {
        const response = await axiosInstance.get(`/api/v1/waktu/${id}`);
...
const WaktuEdit = () => {
    ...
    useEffect(() => {
        if (data) {
            ...
            setWaktuData({
                ...
                kategori_waktu_id: data.kategori_waktu_id || "", // Perbaikan
    ...
    const mutation = useMutation({
        mutationFn: async (waktu) => {
            return await axiosInstance.put(`/api/v1/waktu/${id}`, waktu);
        },
        onSuccess: (response) => {
