...
const fetchKelas = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/kelas');
...
const KelasList = () => {
    ...
    const { data: kelas = [], isLoading, isError, error: queryError } = useQuery({
      ...
      const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const response = await axiosInstance.delete(`/api/v1/kelas/${id}`);
        ...
        <PageContainer title="Kelas" description="Data Kelas">
            <ParentCard title="Kelas">
                ...
                <Box
                    ...
                    />
                   <AddButton
                    icon={<IconPlus size={20} color="white" />} // Mengatur ukuran dengan `size`
                    onClick={handleAdd}
                    >
                        Tambah Kelas
                    </AddButton>
