...
const fetchJadwalMapel = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/jadwal-mapel');
...
const JadwalMapelList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
            const response = await axiosInstance.delete(`/api/v1/jadwal-mapel/${id}`);
    ...
    .filter((mapel) =>
        mapel.nama_kelas.toLowerCase().includes(searchQuery.toLowerCase())
    )
    ...
    return (
        <PageContainer title="Jadwal Mapel" description="Jadwal Mapel">
            <ParentCard title="Jadwal Mata Pelajaran">
                ...
                />
                <AddButton
                    icon={<IconCalendarTime style={{ color: 'white', marginRight: '8px' }} />}
                    onClick={handleAdd}   
                >
                    Tambah Jadwal
                </AddButton>
