...
const fetchUserGuru = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/users/guru');
...
const UserGuruList = () => {
    ...
    const handleEdit = (id) => {
        navigate(`/dashboard/admin-sekolah/user/guru/edit/${id}`);
    };

    const handleDelete = (user_id, type = 'pegawai') => {
        console.log('Menghapus guru dengan data berikut:');
