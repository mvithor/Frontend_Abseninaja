...
import PegawaiStafTable from "src/apps/admin-sekolah/data-staff/List/PegawaiStafTable";
import { useQuery, useMutation, useQueryClient } from "@tanstack/react-query";
import axiosInstance from "src/utils/axiosInstance";

const fetchPegawaiStaf = async () => {
    try {
        const response = await axiosInstance.get('/api/v1/staf');
...
const PegawaiStafList = () => {
    ...
    const deleteMutation = useMutation({
        mutationFn: async (id) => {
          return await axiosInstance.delete(`/api/v1/staf/${id}`);
        },
        onSuccess: () => {
