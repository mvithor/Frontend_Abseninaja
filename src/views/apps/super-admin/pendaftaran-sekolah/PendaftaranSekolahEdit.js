...
import BcPendaftaranEdit from 'src/apps/super-admin/pendaftaran-sekolah/BcPendaftaranEdit';
import PendaftaranEditForm from 'src/apps/super-admin/pendaftaran-sekolah/PendaftaranEdit';
import Alerts from 'src/components/alerts/Alerts';
import { fetchPendaftaranSekolahById, setEditingItem } from 'src/store/apps/pendaftaran-sekolah/PendaftaranSekolahSlice';

const fetchStatusPendaftaran = async () => {
    const response = await axiosInstance.get('/api/v1/status-pendaftaran');
...
const PendaftaranEdit = () => {
    ...
    // Mengambil status pendaftaran dengan React Query
    ...
    const handleSubmit = async (formState) => {
        ...
        try {
            const response = await axiosInstance.put(`/api/v1/pendaftaran/data/${id}`, formState);
            setSuccess(response.data.msg);
            setTimeout(() => {
                navigate('/dashboard/admin/manajemen-sekolah/pendaftaran-sekolah');
              }, 3000);
        } catch (error) {
